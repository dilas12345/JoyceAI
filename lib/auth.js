...
module.exports = (server) => {
  ...
  server.post('/auth/signup', async (req, res) => {
    ...
    if (fullName && email && password) {
      bcrypt.hash(password, saltRounds).then(async function(hash) {
        const results = await addUser(fullName, email, hash)
        if (results && results.length > 0) {
          bcrypt.hash(fullName + email + hash, saltRounds).then(function(hash) {
            sendVerificationEmail(email, "http://" + req.headers.host + "/auth/verify?email=" + email + "&hash=" + hash)
          })
          console.log(res.json({email: email}))
          return res.json({email: email})
        } else {
          console.log("Hello", res.json({email: email}));
          return res.json({message: 'User already exists with email ' + email})
        }
      })
    } else {
      console.log(res.json({message: 'Hello else here'}))
      return res.json({message: 'Please enter email and Password!'})
    }
  })
  ...
  //   try {
  //     const results = await pool.query(`INSERT INTO users (fullName, email, password) VALUES ("${fullName}", "${email}", "${password}");`)
  //     return results
  //   }catch(e){
  //     console.error(e)
  //   }
  // }
  async function addUser(fullName, email, password) {
    let results = null; // add default value for results
    try {
      results = await pool.query(`INSERT INTO users (fullName, email, password) VALUES ("${fullName}", "${email}", "${password}");`);
    } catch(e) {
      console.error(e);
    }
